...
class ParametricModel:
    ...
    """
    def __init__(self, official_model_file: str, use_pose_blendshape=False, device=torch.device('cuda:0'), video_path=None):
        r"""
    ...
    def view_mesh(self, vertex_list: list, fps=60, distance_between_subjects=0.8):
        ...
        faces = np.concatenate(f_list)
        if verts.shape[0] > 1:
            vo3d.render_sequence_3d(verts, faces, 720, 720, self.video_path+'.mp4', fps, visible=True)
        else:
            vo3d.vis_mesh(verts[0], faces)
